  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("poll_options", "Select Polls:",
                  choices = c("All Polls", unique(polls_combined$poll_name)))
    ),
    ...
    data_source <- switch(input$data_source,
                          ...
                          "polls" = {
                            ...
                            if (input$poll_options != "All Polls") {
                              polls <- filter(polls, poll_name == input$poll_options)
                            }
                            return(polls)
                          })
    
    filtered_data <- subset(data_source, date >= input$date_range[1] & date <= input$date_range[2])
